...
class Archives
{
    ...
    function __construct($aid)
    {
        ...
        else
        {
            ...
            {
                // 如果当前文档不是系统模型,为单表模型
                $query = "SELECT arc.*,tp.reid,tp.typedir,tp.sptp,tp.zjtp,tp.zjxmtp1,tp.zjxmtp2,ch.addtable
                FROM `#@__archives` arc
    ...
    $this->Fields['litpic'] = $GLOBALS['cfg_cmspath'].'/images/defaultpic.gif';
        ...
        if($this->Fields['typeid']==36 || $this->Fields['typeid']==143 || $this->Fields['typeid']==144 || $this->Fields['typeid']==145 || $this->Fields['typeid']==146 || $this->Fields['typeid']==147 || $this->Fields['typeid']==148){
        $this->Fields['litpic'] = $GLOBALS['cfg_cmspath'].'/images/nt_defaultpic.jpg';
        }else{
        $this->Fields['litpic'] = $GLOBALS['cfg_cmspath'].'/images/defaultpic.gif';
        }
    }
