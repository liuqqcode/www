...
class WxpayController extends Controller {
   ...
   */
    var $config = array(
         'appid' => "wx15cb0ffdd0c23517",                           /*微信开放平台上的应用id*/
         'mch_id' => "1485850032",                            /*微信申请成功之后邮件中的商户id*/
         'api_key' => "DC357FBF6032F672EA86C92A2888F5F2",              /*在微信商户平台上自己设定的api密钥 32位*/
         'notify_url' =>'http://yf.xnetyy.com/wxpay.php' /*自定义的回调程序地址id*/
    );
	//入口地址，让安卓端请求
    public function pays()
    {
        if (IS_POST) {
            ...
            $reannumb = $this->randomkeys(4).time();   //获取随机的订单号
            $rows = $_POST;
            $price = $_POST['price']*100;
            $rows['create_time'] = time();
            $rows['price'] = $price / 100;
            $rows['order'] = $reannumb;
            $order = M('Order');
            $re = $order->add($rows);
            $response = $this->getPrePayOrder('商品订单支付',$reannumb,$price);
            $x = $this->getOrder($response['prepay_id']);   //返回给客户的二次签名
            $data=array('response'=>$response,'x'=>$x,'code'=>1,orderid=>$re);
    ...
    */
	...
	public function callback(){
        ...
        $result= $order->where(['order' => $id])->find();
        if ($result) {
            $row['state'] = 1;
            $order->where(['order' => $id])->save($row);
        }
		exit('SUCCESS');
	}
