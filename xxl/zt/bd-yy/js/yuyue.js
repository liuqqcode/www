...
function swt_tz() {
    ...
    if (paths.indexOf("zg.") > 0 || paths.indexOf("4g.xnekyy.com") > 0) {
...
function weixin(id, bs, hospital, names, tel) {
    var sourceurls = document.referrer;
    var weburl = window.location.href;
    var sourceurl = sourceurls.split("//")[1];
    var suburl = weburl.split("//")[1];
    ...
    if (phone.value.length != 8 && phone.value.length != 11 && phone.value.length != 12) {
        alert('请您输入正确的电话号码！');
        phone.focus();
        return false;
    } else {
